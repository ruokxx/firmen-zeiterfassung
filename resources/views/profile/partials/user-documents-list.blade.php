<section>
    ...
    <div class="mt-6 space-y-6">
        @forelse($user->documents as $document)
            <div class="p-4 bg-gray-50 rounded-lg border border-gray-200 {{ $document->is_completed ? 'opacity-70' : '' }}">
                <div class="flex justify-between items-start">
                    <div>
                        <h3 class="text-md font-semibold text-gray-800">{{ $document->title }}</h3>
                        <p class="text-sm text-gray-500">Von: {{ $document->creator->name }} | Am: {{ $document->created_at->format('d.m.Y H:i') }}</p>
                ...
                <form method="POST" action="{{ route('user.documents.update', $document) }}" enctype="multipart/form-data" class="mt-4 border-t pt-4">
                    @csrf
                    @method('PATCH')
